...
public class DelivererImpl<T> implements Deliverer {
  ...
  private void deliverInternal(String toList, InputArea inputArea,
    ...
    //    
    remoteIPs = new String[recipientsList.length];

    message = composeMessage(recipientsList, inputMessage);

    logArea.lock();
    cursorControl.setBusy(true);
    logArea.appendDate();

    deliverOk = deliverMessage(recipientsList, remoteIPs, message);
    deliveredLog = completeLog(deliverOk, inputMessage, inputArea);
  ...
  private boolean isValidOperation(String[] recipients, String message) {
    ...
    if (recipients.length == 0)
      return false;
    if (isBroadcast(recipients[0]) && message.length() == 0)
      return false;
    return true;
  }
